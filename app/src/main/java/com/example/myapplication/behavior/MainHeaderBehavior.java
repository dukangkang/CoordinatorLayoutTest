...
import com.example.myapplication.source.ViewOffsetBehavior;
...
public class MainHeaderBehavior extends ViewOffsetBehavior<View> {
    ...
    public boolean onInterceptTouchEvent(CoordinatorLayout parent, final View child, MotionEvent ev) {
        Log.e("dkkkk", "MainHeaderBehavior onInterceptTouchEvent action = " + ev.getAction());
        switch (ev.getAction()) {
...
//            consumed[1] = dy;
//        } else
            if (target instanceof CustomRecyclerView) {//处理列表滑动
            CustomRecyclerView recyclerView = (CustomRecyclerView) target;
